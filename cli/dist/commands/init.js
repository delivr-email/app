...
import { isDockerRunning, isPortAvailable, getDelivrDir, isDelivrInitialized, execLive, generatePassword, exec, } from "../utils.js";
...
async function checkPrerequisites(apiPort) {
    if (!isDockerRunning()) {
        console.error(chalk.red("Docker is not running. Install it from https://docs.docker.com/get-docker/"));
        process.exit(1);
    }
    const portsToCheck = [apiPort, 5432, 6379];
    for (const port of portsToCheck) {
        if (!(await isPortAvailable(port))) {
            console.warn(chalk.yellow(`Warning: port ${port} is already in use.`));
        }
    }
}
function generateFiles(dir, answers) {
    const templatesDir = getTemplatesDir();
    const composeTemplate = answers.mode === "local"
        ? "docker-compose.local.yml"
        : "docker-compose.prod.yml";
    const envTemplate = answers.mode === "local" ? "env.local.template" : "env.prod.template";
    ...
    copyFileSync(resolve(templatesDir, composeTemplate), resolve(dir, "docker-compose.yml"));
...
export async function initCommand() {
    ...
    if (isDelivrInitialized()) {
        console.error(chalk.red("Delivr is already initialized in this directory. Use 'delivr start' to start it."));
        process.exit(1);
    }
